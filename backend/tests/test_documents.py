...
async def test_document_upload_and_status():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        status_resp = await client.get(f"/api/documents/{doc_id}")
...
async def test_document_upload_invalid_type():
    """Test uploading non-PDF document returns 400 bad request error."""
...
async def test_document_upload_file_too_large():
    ...
    # Create content larger than max allowed bytes (using setting)
...
async def test_document_status_not_found():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get("/api/documents/non_existent_id")
